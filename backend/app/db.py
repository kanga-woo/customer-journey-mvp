...
conn = sqlite3.connect(DB_PATH)
c = conn.cursor()
c.execute("SELECT event_type, properties, timestamp FROM events WHERE customer_id=? ORDER BY timestamp DESC LIMIT ?", (customer_id, limit))
rows = c.fetchall()
conn.close()
...
conn = sqlite3.connect(DB_PATH)
c = conn.cursor()
c.execute("SELECT timestamp FROM events WHERE customer_id=? ORDER BY timestamp DESC", (customer_id,))
rows = c.fetchall()
conn.close()
from datetime import datetime
if not rows:
